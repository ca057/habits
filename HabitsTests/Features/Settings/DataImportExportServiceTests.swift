...
struct DataImportExportServiceTests {
    ...
    struct DataImportTests {
        ...
        @Test("imports backup file with entries containing a date property")
        func importsBackupFileWithEntriesWithDate() throws {
            let backupJson = """
                {
                    "appVersion": "1.2.3",
        ...
        @Test("imports backup file with entries containing a day property")
        func importsBackupFileWithEntriesWithDay() throws {
            let backupJson = """
                {
                    "appVersion": "1.2.3",
                    "exportDate": "2026-04-21T23:00:00Z",
        ...
        @Test("fails gracefully when importing a backup file where entries have neither a day nor date property")
        func failsForImportOfBackupFileWithEntriesWithNeitherDateOrDay() throws {
            let backupJson = """
                {
                    "appVersion": "1.2.3",
                    "exportDate": "2026-04-21T23:00:00Z",
            ...
            #expect(items.count == 1)
            #expect(items.first?.entries.count == 0)
        }
    }
